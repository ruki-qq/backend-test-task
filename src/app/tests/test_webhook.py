    ...
    async def _test(client: AsyncClient, **kwargs):
        channel = Channel(
            ...
            settings=ChannelSettings(
                webhook_url="https://example.com/webhook", token="channel_token"
            ),
            is_active=True,
        )
        ...
        with patch(
            "app.services.chat_service.ChatService.send_message_to_channel"
        ) as mock_send:
...
def test_webhook_message_invalid_token():
    ...
    async def _test(client: AsyncClient, **kwargs):
        ...
        headers = {"chat_bot_authorization": "Bearer invalid_token"}

        response = await client.post(
            "/api/webhook/new_message", json=message_data, headers=headers
        )
        invalid_token = headers["chat_bot_authorization"][7:]

        assert response.status_code == 401
        assert (
            f"hatBot with chat bot token '{invalid_token}' not found"
...
def test_webhook_message_missing_authorization():
    """Тест webhook без заголовка chat_bot_authorization"""
    ...
    async def _test(client: AsyncClient, **kwargs):
        ...
        assert response.status_code == 401
        assert "chat_bot_authorization header is required" in response.json()["detail"]
...
def test_webhook_message_invalid_authorization_format():
    ...
    async def _test(client: AsyncClient, **kwargs):
        ...
        headers = {"chat_bot_authorization": "InvalidFormat test_token_123"}
